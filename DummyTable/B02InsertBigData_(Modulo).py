...
def populate_tables():
    ...
    try:
        cursor.execute("""
            ...
            SELECT i, i % 10 FROM generate_series(0, 2999999) AS s(i);
        """)
        ...
        cursor.execute("""
            ...
            SELECT i, i % 10 FROM generate_series(0, 4999999) AS s(i);
        """)
        connection.commit()

        cursor.execute("""
            INSERT INTO table3 (nkey, ndependency)
            SELECT i, i % 10 FROM generate_series(0, 6999999) AS s(i);
        """)
        connection.commit()
