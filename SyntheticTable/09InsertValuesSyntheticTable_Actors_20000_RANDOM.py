...
def generate_and_insert_data():
    ...
    # 101 häufige Jahre nach dem Jahr 2000, die 80 % der 20.000 Einträge ausmachen (16.000 Einträge)
    ...
    frequent_count_per_year = 16000 // len(frequent_years)  # Ungefähr gleiche Anzahl pro Jahr
    for year in frequent_years:
        frequent_entries.extend([year] * frequent_count_per_year)

    # Den Rest der 16.000 auf zufällige Jahre verteilen (falls 16.000 nicht gleichmäßig durch 101 teilbar sind)
    remaining_frequent = 16000 - len(frequent_entries)
    for i in range(remaining_frequent):
        frequent_entries.append(random.choice(frequent_years))

    # 3998 weniger häufige Geburtsjahre, die jeweils einmal vorkommen
    less_frequent_years = list(map(int, np.random.choice(range(1900, 2000), 3998, replace=True)))
    ...
    # Stelle sicher, dass genau 20.000 Geburtsjahre vorhanden sind
    while len(birth_years) < 20000:
